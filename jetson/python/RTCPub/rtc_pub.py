...
from RTCCam import WebCam, CSICam
...
class RTCWebCamPublisher(RTCDataChannel):
    def __init__(
        self,
        channel="c0dko1upjh69daabh9pg",
        call_back_on_close=None,
        call_back_on_error=None,
    ):
        print(f"RTCWebCamPublisher {channel}")
        super().__init__(channel=channel)
        w = 640
        h = 480
        self._cam_f = WebCam(width=w, height=h, cameranumber=1, camID=0)
        # self._cam_r = WebCam(width=w, height=h, cameranumber=2, camID=1)
        # self._cam_m = WebCam(width=w, height=h, cameranumber=3, camID=2)
        # self._cam_i = WebCam(width=w, height=h, cameranumber=4, camID=3)
        # self._peer.video.putSubscription(self._cam_f)  # 카메라 설정(전방)
        self._peer.video.addTrack(
            frameSubscription=self._cam_f, fps=30
        )  # 카메라 비디오트랙 추가(전방)
        # self._peer.video.addTrack(frameSubscription=self._cam_r, fps=30)  # 카메라 비디오트랙 추가(전방)
        # self._peer.video.addTrack(frameSubscription=self._cam_m, fps=30)  # 카메라 비디오트랙 추가(전방)
        # self._peer.video.addTrack(frameSubscription=self._cam_i, fps=30)  # 카메라 비디오트랙 추가(전방)
        # self._peer.video.putSubscription(self._cam_r)  # 카메라 설정(후방)
        # self._peer.video.addTrack(frameSubscription=self._cam_r, fps=30)  # 카메라 비디오트랙 추가(후방)

        # @self._peer.video.onClose
        # def video_on_close(err):
        #     print('-----video on close')
        #
        # @self._peer.video.onError
        # def video_on_error(err):
        #     print('-----video on error')
        self.reg_event_close()
        self.reg_event_error()

    async def reg_event_close(self):
        video_error = await self._peer.video.onClose()
        print("-----video on close")

    async def reg_event_error(self):
        video_error = await self._peer.video.onError()
        print("-----video on error")

    def peer(self) -> RTCConnection:
        return self._peer
    ...
    def __del__(self):
        try:
            ...
            print("-----del RTCWebCamPublisher exception:%s" % e)

        return super().__del__()

    async def sender(self, q=None):
        print("-----sender")
        # while True:
        # val = await self._q.get()
        # val = "Fuck"
        # self._dc_stream.put_nowait(val)
        # await self._loop.run_in_executor(None, self._dc_stream.put_nowait, val)
...
class RTCCSICamPublisher(RTCDataChannel):
    def __init__(self, channel="c0dko1upjh69daabh9pg", cameranumber=0):
        print(f"RTCCSICamPublisher {channel}")
        super().__init__(channel=channel)
